...
_session.Session.set('zid', "yet to be unlocked");
_session.Session.set('zad', "none currently");
_session.Session.set('ismember', "");
...
_templating.Template.body.helpers({
    zid: function zid() {
        ...
        return _session.Session.get('zid');
    },
    zad: function zad() {
        // this would return a reactive var value
        return _session.Session.get('zad');
    },
    ...
    ismember: function ismember() {
        // this would return a reactive var value
        return _session.Session.get('ismember');
    }
});

_templating.Template.body.events({
    'submit .create-zone': function submitCreateZone(event) {
        ...
        var zid = '0xd90b704c738b49ff50741bd887334e49d8f277f1';
        ...
        if (typeof Keystore != 'undefined') {
            ...
            zoneFactory.new({
                ...
                if (typeof contract.address != 'undefined') {
                    ...
                    Zone = contract;
                    _session.Session.set('zad', contract.address);
                }
            });
        }
    },
    'submit .set-member': function submitSetMember(event) {
        // Prevent default browser form submit
        event.preventDefault();

        var target = event.target;
        //todo: should probably check address for member is valid
        var members = [target.member.value];
    ...
    'submit .is-member': function submitIsMember(event) {
        // Prevent default browser form submit
        event.preventDefault();

        var target = event.target;
        //todo: should probably check address for member is valid
        var member = target.ismember.value;
        ...
        if (typeof Zone != 'undefined') {
            Zone.isMember(member, function (e, result) {
                if (!e) {
                    ...
                    if (result) {
                        _session.Session.set('ismember', "They are a member");
                    } else {
                        _session.Session.set('ismember', "They are not a member");
                    }
                }
            });
        } else {
    ...
    'submit .set-credential': function submitSetCredential(event) {
        // Prevent default browser form submit
        event.preventDefault();

        // Get value from form element
        var target = event.target;
        var seed = target.seed.value;
        ...
        _ethLightwallet2.default.keystore.deriveKeyFromPassword(password, function (err, pwDerivedKey) {

            Keystore = new _ethLightwallet2.default.keystore(seed, pwDerivedKey);
            ...
            _session.Session.set('zid', Keystore.getAddresses()[0]);
        });
    }
});
